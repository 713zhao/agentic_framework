# Important!!!
#
# Set your 'Gemini_key' as an environment variable
#
# <---- You will also likely want to use the "folder" icon to add some files
#       for the agent to look at
#
import os
# The Gemini API key is already set in environment variable as 'Gemini_key'
# LiteLLM will automatically use GEMINI_API_KEY environment variable
...
def generate_response(prompt: Prompt) -> str:
    ...
    if not tools:
        ...
        if response.choices and len(response.choices) > 0:
            ...
            print(f"Warning: Empty response from LLM. Response: {response}")
            result = json.dumps({"tool": "terminate", "args": {"message": "Error: Empty response from LLM"}})
